    ...
    res.on('end', () => {
      try {
        ...
        if (nodes.length > 0) {
          ...
          console.log('   Sample country:', nodes[0].country, typeof nodes[0].country);
          console.log('   Sample node_id:', nodes[0].node_id, typeof nodes[0].node_id);
        }
      ...
      res.on('end', () => {
        try {
          ...
          if (channels.length > 0) {
            ...
            console.log('   Sample capacity:', channels[0].capacity, typeof channels[0].capacity);
          }
...
function simulateFrontendProcessing(nodes, channels) {
  ...
  function getCountryCode(country) {
    ...
    const codes = {
      'United States': 'US', 'Germany': 'DE', 'Hong Kong': 'HK', 'Singapore': 'SG',
      'Australia': 'AU', 'South Africa': 'ZA', 'Brazil': 'BR', 'Indonesia': 'ID',
      'Japan': 'JP'
    };
  ...
  geoNodes.forEach(item => {
    console.log(`   ${item.countryCode}: ${item.nodeCount} (type: ${typeof item.nodeCount}), ${item.totalCapacity} (type: ${typeof item.totalCapacity})`);
  });
  ...
  if (geoNodes.length > 0) {
    ...
    console.log('   VisualMap max:', geoNodes.length > 0 ? Math.max(...geoNodes.map(item => item.nodeCount)) : 1);
  }
  ...
  const hasUndefined = geoNodes.some(item => 
  ...
  const hasNaN = geoNodes.some(item => 
    isNaN(item.nodeCount) || 
    isNaN(item.totalCapacity)
  );

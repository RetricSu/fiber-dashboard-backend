  ...
  const nodesReq = http.request({
    ...
    res.on('end', () => {
      try {
        ...
        const countries = nodesData.filter(n => n.country && n.country !== 'Unknown');
        console.log('   Countries with data:', countries.length);
        countries.slice(0, 5).forEach(n => {
          console.log(`   - ${n.country}: ${n.node_name || n.node_id.substring(0, 16)}...`);
        });
    ...
    const channelsReq = http.request({
      ...
      res.on('end', () => {
        try {
          ...
          console.log('\n🔍 Full data flow:');
          console.log('1. Nodes with countries:', nodesData.filter(n => n.country).length);
          ...
          console.log('5. Total capacity from channels:', totalCapacity.toFixed(2), 'CKB');
          ...
          geoNodes.forEach(c => {
            console.log(`   ${c.countryCode}: ${c.nodeCount} nodes, ${c.totalCapacity.toFixed(2)} CKB`);
          });

        ...
        const channelsReq = http.request({
          ...
          res.on('end', () => {
            try {
              ...
              console.log('mapData types:', mapData.map(item => ({
              ...
              const testMapData = [
                { name: 'United States', value: 106, capacity: 200100.42 },
              ...
              console.log('All zero capacity:', geoNodes.every(item => item.totalCapacity === 0));
              console.log('Has negative values:', geoNodes.some(item => item.totalCapacity < 0));

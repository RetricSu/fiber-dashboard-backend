                ...
                try {
                  ...
                  channels.forEach((channel, i) => {
                    if (i < 5) {
                      ...
                      console.log(
                        `   Channel ${i + 1}: capacity=${channel.capacity}`
                      );
                    }
                  });
                    ...
                    try {
                      ...
                      const capacityInShannons =
                        typeof capacity === "string"
                          ...
                          : BigInt(capacity);
                      const capacityInCKB =
                        Number(capacityInShannons) / SHANNONS_PER_CKB;
                      ...
                      console.log(
                        `     Node1: ${channel.node1} -> ${node1Country}`
                      );
                      console.log(
                        `     Node2: ${channel.node2} -> ${node2Country}`
                      );
                      ...
                      if (node1Country) {
                        ...
                        countryCapacity.set(
                          node1Country,
                          countryCapacity.get(node1Country) + capacityInCKB / 2
                        );
                      }
                      if (node2Country) {
                        ...
                        countryCapacity.set(
                          node2Country,
                          countryCapacity.get(node2Country) + capacityInCKB / 2
                        );
                      }
                    } catch (e) {
                  ...
                  console.log("\n👥 Country node counts:");
                  const sortedNodeCounts = Array.from(
                    countryNodes.entries()
                  ).sort((a, b) => b[1] - a[1]);

                  sortedNodeCounts.forEach(([country, count]) => {
                    const capacity = countryCapacity.get(country) || 0;
                    console.log(
                      `   ${country}: ${count} nodes, ${capacity.toFixed(2)} CKB`
                    );
                  });
                } catch (e) {
